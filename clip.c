...
int main(int argc, char** argv) 
{
	...
	int paddingImageSize = (bmpInfoHeader.biWidth+2)*elemSize * (bmpInfoHeader.biHeight+2);
    ...
    outimg = (ubyte*)malloc(sizeof(ubyte)*imageSize);
   	paddingimg = (ubyte*)malloc(sizeof(ubyte)*paddingImageSize); 
	fread(paddingimg, sizeof(ubyte), imageSize, fp); 
    ...
    memset(outimg, 0, sizeof(ubyte)*imageSize);
   	memset(paddingimg, 0, sizeof(ubyte)*paddingImagesize); 
	...
	for(y = 1; y < bmpInfoHeader.biHeight - 1; y++) { 
        for(x = 1; x < (bmpInfoHeader.biWidth - 1) * elemSize; x+=elemSize) {
            for(z = 0; z < elemSize; z++) {
                ...
                int b = inimg[x+y*size+z];
	int g = inimg[x+y*size+z];
	int r = inimg[x+y*size+z];

	outimg[x+y*size+z]= clip(b, 0, 255);
	outimg[x+y*size+z]= clip(g, 0, 255);
	outimg[x+y*size+z]= clip(r, 0, 255);
   ...
   /* 
	for(y = 0; y < bmpInfoHeader.biHeight; y++){ 
		for(x = 0; x < (bmpInfoHeader.biWidth) * elemSize; x+=elemSize) {
			for(z=0; z<3; z++){
				if(y==0 ||  x==0 || y == bmpInfoHeader.biHeight-1 || x==(bmpInfoHeader.biWidth-1)*elemSize)
					outimg[x+y*size+z] = 0;
