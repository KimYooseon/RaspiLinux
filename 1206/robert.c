    ...
    for(y = 1; y < bmpInfoHeader.biHeight + 1; y++) { 
        for(x = elemSize; x < padSize; x+=elemSize) {
            for(z = 0; z < elemSize; z++) {
                ...
                for(int i = -1; i < 2; i++) {
                    for(int j = -1; j < 2; j++) {
                        x_val += x_kernel[i+1][j+1]*padimg[(x+i*elemSize)+(y+j    )*padSize+z];
						y_val += y_kernel[i+1][j+1]*padimg[(x+i*elemSize)+(y+j    )*padSize+z];

					}
                }
				//quicksort(median,0,8);  
				//outimg[(x-elemSize)+(y-1)*size+z] = LIMIT_UBYTE(x_val);
				outimg[(x-elemSize)+(y-1)*size+z] = LIMIT_UBYTE(sqrt(x_val*x_val+y_val*y_val));
